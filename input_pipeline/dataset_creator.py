...
from input_pipeline.dataset import LidarData
    ...
    def generate_pairs(self, threshold):
        ...
        for i, pose in enumerate(poses):
            try:
                pair_dict = dict()
                pair_dict["source"] = i
                target_scan_path = os.path.join(self.config['data_dir'], str(i))
                ...
                for ix in index[0]:
                    try:
                        source_scan_path = os.path.join(self.config['data_dir'], str(ix))
