...
class BasicUpdateBlock(nn.Module):
    def __init__(self, corr_channels, hidden_dim, downsample_factor=8, learn_upsample=True,
                 **kwargs):
        ...
        if learn_upsample:
            self.mask = nn.Sequential(
                ...
                nn.Conv2d(256, downsample_factor * 9, 1, padding=0))

    def forward(self, net, inp, corr, flow, upsample=True, **kwargs):
        motion_features = self.encoder(flow, corr)
        inp = torch.cat([inp, motion_features], dim=1)  # [1, 160, 32, 128]
